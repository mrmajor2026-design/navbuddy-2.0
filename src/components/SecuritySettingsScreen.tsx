...
import { ArrowLeft, Lock, Shield, Smartphone, Key, Bell } from "lucide-react";
...
export default function SecuritySettingsScreen({ onBack }: SecuritySettingsScreenProps) {
  const securityOptions = [
    { label: "Password", value: "Last changed 3 months ago", icon: Key },
    { label: "Two-Factor Auth", value: "Enabled", icon: Smartphone },
    { label: "Biometric Login", value: "Face ID Enabled", icon: Shield },
    { label: "Login Alerts", value: "Email & Push", icon: Bell },
  ];
          ...
          {securityOptions.map((option, index) => (
            <div key={index} className="flex items-start gap-4 p-4 rounded-2xl bg-gray-50">
              <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center shadow-sm">
                <option.icon size={20} className="text-[#FF6B35]" />
              </div>
              <div className="flex-1">
                <p className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-1">{option.label}</p>
                <p className="text-lg font-semibold text-[#0F172A]">{option.value}</p>
              </div>
            </div>
          ))}
        </div>
